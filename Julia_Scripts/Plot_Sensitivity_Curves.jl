...
function sensitivity_curves(model_variable::String, df::DataFrame, mat_SIF::Array, mat_REF::Array, wlf::Array, wl::Array)
    ...
    if mat_SIF == mat_SIF_sunlit || mat_SIF == mat_SIF_shaded || mat_SIF == mat_SIF_scattered || mat_SIF == mat_SIF_soil
        ...
        if mat_SIF == mat_SIF_sunlit
            plot_title = "CliMA Top of Canopy Outgoing SIF and SW Radiation - SIF Sunlit"
            curve_sif757    = plot(df[:, 1], df.SIF757_sunlit, ylabel          = "SIF₇₅₇ (mW m⁻² nm⁻¹ sr⁻¹)",
                                xlabel = xlab, linewidth = 2, legend = false, marker = 5, framestyle = :box)
        elseif mat_SIF == mat_SIF_shaded
            plot_title = "CliMA Top of Canopy Outgoing SIF and SW Radiation - SIF Shaded"
            curve_sif757    = plot(df[:, 1], df.SIF757_shaded, ylabel          = "SIF₇₅₇ (mW m⁻² nm⁻¹ sr⁻¹)",
                                xlabel = xlab, linewidth = 2, legend = false, marker = 5, framestyle = :box)
        elseif mat_SIF == mat_SIF_scattered
            plot_title = "CliMA Top of Canopy Outgoing SIF and SW Radiation - SIF Scattered"
            curve_sif757    = plot(df[:, 1], df.SIF757_scattered, ylabel          = "SIF₇₅₇ (mW m⁻² nm⁻¹ sr⁻¹)",
                                xlabel = xlab, linewidth = 2, legend = false, marker = 5, framestyle = :box)
        elseif mat_SIF == mat_SIF_soil
            plot_title = "CliMA Top of Canopy Outgoing SIF and SW Radiation - SIF Soil"
            curve_sif757    = plot(df[:, 1], df.SIF757_soil, ylabel          = "SIF₇₅₇ (mW m⁻² nm⁻¹ sr⁻¹)",
                                xlabel = xlab, linewidth = 2, legend = false, marker = 5, framestyle = :box)
        end
    else
        l = @layout([a{0.01h}; b c d; e f])
        title = "CliMA Top of Canopy Outgoing SIF and SW Radiation"
        # Curves
