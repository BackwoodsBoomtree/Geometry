...
function derive_spectrum(input_data, source_clump, source_lai, source_cab, month, week)
    FT = Float32;
    ENV["GKSwstype"]="100";
    ...
    # Clumping
    if source_clump == "LUT"
        CLI_LUT = load_LUT(ClumpingIndexMODIS{FT}(), "240X", "1Y");
        mask_LUT!(CLI_LUT, FT[0,1]);
        # CLI_LUT = regrid_LUT(CLI_LUT, 2)
    end
    # Cab
    if source_cab == "LUT"
    ...
    for i in eachindex(input_data.VZA)
        ...
        if source_clump == "LUT"
            can.Ω = read_LUT(CLI_LUT, FT(input_data.latitude[i]), FT(input_data.longitude[i]));
        end
        print("Clumping ", can.Ω, " ")

        if source_lai == "LUT"
            can.LAI = read_LUT(LAI_LUT, FT(input_data.latitude[i]), FT(input_data.longitude[i]), month);
            can.iLAI = can.LAI * can.Ω / can.nLayer;
        else
            can.LAI = source_lai;
            can.iLAI = source_lai * can.Ω / can.nLayer;
        end
        print("LAI ", can.LAI, " ")
