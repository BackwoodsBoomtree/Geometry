...
amazon_data_path   = "C:/Russell/Projects/Geometry/R_Scripts/CSV/sif_ATTO_Tower_Manaus_Brazil_(incorrect)_2020-06-26_6500_PAR_Adjusted.csv";
amazon_cab         = 60;
amazon_week        = NaN;
amazon_clumping    = 1;
amazon_sif_yield   = 1;
...
CSV.write(amazon_file_name*"_simulated_par_adjusted.csv", amazon_data);
CSV.write(amazon_file_name*"_means_par_adjusted.csv", amazon_mean);
CSV.write(amazon_file_name*"_stats_par_adjusted.csv", amazon_stats);
...
p_amazon = scatter(amazon_mean.mean_sif757_sim, amazon_mean.mean_sif757, xlabel = "Mean CliMA SIF (mW m⁻² µm⁻¹ sr⁻¹)", ylabel = "Mean OCO3 SIF (mW m⁻² µm⁻¹ sr⁻¹)", reg = true, linewidth = 2,
            title = "Amazon Rainforest in Guyana", titlefontsize = 13, legend = :topleft, label = "757 nm", framestyle = :box, yerror = amazon_mean.stderr_sif757)
...
annotate!(1.05, 1.1, text(string("R² = ", amazon_stats.r2_sif757[1], "\nMAE = ", amazon_stats.mae_sif757[1], "\n", pval_sif757), :left, 12))
annotate!(0.55, 1.1, text(string("R² = ", amazon_stats.r2_sif771[1], "\nMAE = ", amazon_stats.mae_sif771[1], "\n", pval_sif771), :left, 12))

savefig(p_amazon, "C:/Russell/Projects/Geometry/Julia_Scripts/Figures/OCO3/Regression_Plot_OCO3_CLIMA_Amazon.pdf")
