...
poly_df2 <- SpatialPolygonsDataFrame(pollyLayer,df_ATTO)

# CRS
proj4string(poly_df2) <- CRS("+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0")
...
m <- leaflet(poly_df2, width = 550, height = 700, options = leafletOptions(zoomControl = T, attributionControl=F)) %>% 
  addProviderTiles(providers$Esri.WorldImagery) %>%
  addPolygons(data=poly_df2, stroke = FALSE, smoothFactor = 0.2, fillOpacity = 1, color = ~vPal(SIF740),
              popup = paste("Sounding ID: ", poly_df2$ID, "<br>",
                            "Time: ", poly_df2$Time, "<br>",
                            "SIF740nm Daily: ", round(poly_df2$SIF740_D,digits=2), "W/m^2/sr/µm<br>",
                            "SIF757nm Daily: ", round(poly_df2$SIF757_D,digits=2), "W/m^2/sr/µm<br>",
                            "SIF771nm Daily: ", round(poly_df2$SIF771_D,digits=2), "W/m^2/sr/µm<br>",
                            "SIF740nm: ", round(poly_df2$SIF740,digits=2), "W/m^2/sr/µm<br>",
                            "SIF740nm Uncertainty: ", round(poly_df2$SIF740_U,digits=2), "W/m^2/sr/µm<br>",
                            "Viewing Zenith Angle: ", round(poly_df2$VZA,digits=2), "<br>",
                            "Viewing Azmuth Angle: ", round(poly_df2$VAA,digits=2), "<br>",
                            "Solar Zenith Angle: ", round(poly_df2$SZA,digits=2), "<br>",
                            "Solar Azmuth Angle: ", round(poly_df2$SAA,digits=2), "<br>",
                            "Humidity: ", round(poly_df2$Humidity,digits=4), "kg/kg<br>",
                            "Surface Pressure: ", round(poly_df2$Surface_Pressure,digits=2), "Pa<br>",
                            "Temperature Skin: ", round(poly_df2$Temp_Skin,digits=2), "K<br>",
                            "Temperature 2m: ", round(poly_df2$Temp_2m,digits=2), "K<br>",
                            "VPD: ", round(poly_df2$VPD,digits=2), "Pa<br>",
                            "Wind Speed: ", round(poly_df2$Wind,digits=2), "m/s<br>",
                            "IGBP Class: ", poly_df2$IGBP_Class))  %>%
  addLegend("bottomright", pal = vPal_rev, values = ~SIF740, title = "SIF740nm<br>W/m^2/sr/µm", opacity = 1,
