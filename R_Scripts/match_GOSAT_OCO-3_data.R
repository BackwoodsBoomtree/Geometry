...
files_oco3  <- list.files(path = "C:/Russell/Projects/Geometry/Data/oco3", pattern = "*.nc4", full.names = TRUE, recursive = TRUE)

pair_files <- function(files_gosat, files_oco3) {
  dates_gosat <- vector()
  dates_oco3  <- vector()
  ...
  for (i in 1:length(files_gosat)){
    ...
    for (i in 1:length(files_oco3)){
    date       <- substr(basename(files_oco3[i]), 12, 17)
    dates_oco3 <- c(dates_oco3, date)
  }
    # Combine dates and filenames into dataframes
  files_gosat <- data.frame(dates_gosat, files_gosat)
  files_oco3  <- data.frame(dates_oco3, files_oco3)
    # Pair the filenames
  paired_files <- data.frame(gosat = vector(), oco3 = vector())
  for (i in 1:nrow(files_gosat)){
    for (j in 1:nrow(files_oco3)){
      if (files_gosat$dates_gosat[i] == files_oco3$dates_oco3[j]){
        paired_files[i, 1] <- files_gosat[i, 2]
        paired_files[i, 2] <- files_oco3[j, 2]
      }
    }
  }
...
match_soundings <- function(paired_file_list) {
  ...
  matched_oco3_sounding_list  <- data.frame()
  ...
  for (i in 1:nrow(paired_file_list)){
    ...
    df_oco3  <- build_data(paired_file_list$oco3[i], "OCO")

      # Subset by mode, cloud flag, and qc flag
    df_gosat <- subset_flags(df_gosat, 0, 0, 0, "GOSAT")
    df_oco3  <- subset_flags(df_oco3, 0, 0, 0, "OCO")

    if (nrow(df_gosat) != 0 && nrow(df_oco3) != 0){
        # Transform dfs into SpatialPointsDataFrame
        # For some reason, SpatialPoints function adds lat/lon columns to output, so we deduct it from input data to avoid duplication for OCO data,
      ...
      coords_oco3            <- as.data.frame(cbind(df_oco3$longitude, df_oco3$latitude))
      colnames(coords_oco3)  <- c("longitude", "latitude")
      point_df_oco3          <- SpatialPointsDataFrame(coords_oco3, proj4string = CRS("+init=epsg:4326"), data = subset(df_oco3, select = -c(longitude, latitude)), coords.nrs = c(7, 8))
      ...
      matched_gosat_sounding_list <- rbind(matched_gosat_sounding_list, as.data.frame(intersect(polydf_gosat, point_df_oco3)))
      matched_oco3_sounding_list  <- rbind(matched_oco3_sounding_list, as.data.frame(intersect(point_df_oco3, polydf_gosat)))

      print(paste0("In total there are now ", nrow(matched_gosat_sounding_list), " overlapping GOSAT soundings, and ", nrow(matched_oco3_sounding_list), " overlapping OCO-3 soundings."))
    } else {
      print(paste0("Day ", date, " excluded due to empty dataframe. GOSAT nrow = ", nrow(df_gosat), "; OCO3 nrow =  ", nrow(df_oco3), "."))
    }
  }
  return(list(matched_gosat_sounding_list, matched_oco3_sounding_list))
}
remove_non_veg <- function (matched_gosat_sounding_list, matched_oco3_sounding_list) {
  ...
  new_matched_oco3_sounding_list  <- matched_oco3_sounding_list
  count <- 0

  for (i in 1:nrow(matched_oco3_sounding_list)) {
    if (matched_oco3_sounding_list$IGBP_index[i] == 13 || matched_oco3_sounding_list$IGBP_index[i] == 15 || matched_oco3_sounding_list$IGBP_index[i] == 16 || matched_oco3_sounding_list$IGBP_index[i] == 17) {
      bad_gosat <- matched_oco3_sounding_list$SoundingID.1[i]

      if (bad_gosat %in% new_matched_oco3_sounding_list$SoundingID.1) { # gosat sounding may have already been removed, so check first
        new_matched_gosat_sounding_list <- subset(new_matched_gosat_sounding_list, SoundingID != bad_gosat)
        new_matched_oco3_sounding_list  <- subset(new_matched_oco3_sounding_list, SoundingID.1 != bad_gosat)
        count <- count + 1
  ...
  return(list(new_matched_gosat_sounding_list, new_matched_oco3_sounding_list))
}
collapse_soundings <- function(matched_gosat_sounding_list, matched_oco3_sounding_list) {
    # Standard error of the mean
  se <- function(x) sd(x) / sqrt(length(x))

  # Subset OCO3 soundings by GOSAT sounding and calculate mean SIF and SE of the mean, and mean Uncertainty then add it to GOSAT sounding list
  for (i in 1:length(unique(matched_oco3_sounding_list$SoundingID.1))) {

    sub_data <- subset(matched_oco3_sounding_list, SoundingID.1 == unique(matched_oco3_sounding_list$SoundingID.1)[i])
    ...
    matched_gosat_sounding_list$Mean_OCO_SIF_Uncertainty_740nm[i] <- mean(sub_data$SIF_Uncertainty_740nm)
    matched_gosat_sounding_list$Mean_OCO_SIF_757nm[i]             <- mean(sub_data$SIF_757nm)
    matched_gosat_sounding_list$Mean_OCO_SIF_Uncertainty_757nm[i] <- mean(sub_data$SIF_Uncertainty_757nm)
    matched_gosat_sounding_list$Mean_OCO_SIF_771nm[i]             <- mean(sub_data$SIF_771nm)
    matched_gosat_sounding_list$Mean_OCO_SIF_Uncertainty_771nm[i] <- mean(sub_data$SIF_Uncertainty_771nm)
...
#region ################# RESULTS ##################
paired_file_list <- pair_files(files_gosat, files_oco3)

matched_sounding_list       <- match_soundings(paired_file_list)
matched_gosat_sounding_list <- as.data.frame(matched_sounding_list[1])
matched_oco3_sounding_list  <- as.data.frame(matched_sounding_list[2])


unfiltered_matched_sounding_means <- collapse_soundings(matched_gosat_sounding_list, matched_oco3_sounding_list)

veg_matched_sounding_list       <- remove_non_veg(matched_gosat_sounding_list, matched_oco3_sounding_list)
veg_matched_gosat_sounding_list <- as.data.frame(veg_matched_sounding_list[1])
veg_matched_oco3_sounding_list  <- as.data.frame(veg_matched_sounding_list[2])

veg_matched_sounding_means <- collapse_soundings(veg_matched_gosat_sounding_list, veg_matched_oco3_sounding_list)
...
write.csv(unfiltered_matched_sounding_means,"C:/Russell/Projects/Geometry/R_Scripts/Figures/Match_GOSAT_OCO/CSV/OCO3/unfiltered_matched_sounding_means_oco3.csv", row.names = FALSE)

write.csv(matched_gosat_sounding_list,"C:/Russell/Projects/Geometry/R_Scripts/Figures/Match_GOSAT_OCO/CSV/OCO3/matched_gosat_oco3_sounding_list.csv", row.names = FALSE)
write.csv(matched_oco3_sounding_list,"C:/Russell/Projects/Geometry/R_Scripts/Figures/Match_GOSAT_OCO/CSV/OCO3/matched_oco3_sounding_list.csv", row.names = FALSE)

write.csv(veg_matched_gosat_sounding_list,"C:/Russell/Projects/Geometry/R_Scripts/Figures/Match_GOSAT_OCO/CSV/OCO3/veg_matched_gosat_oco3_sounding_list.csv", row.names = FALSE)
write.csv(veg_matched_oco3_sounding_list,"C:/Russell/Projects/Geometry/R_Scripts/Figures/Match_GOSAT_OCO/CSV/OCO3/veg_matched_oco3_sounding_list.csv", row.names = FALSE)

write.csv(veg_matched_sounding_means,"C:/Russell/Projects/Geometry/R_Scripts/Figures/Match_GOSAT_OCO/CSV/OCO3/veg_matched_sounding_means_oco3.csv", row.names = FALSE)
write.csv(time_veg_matched_sounding_means,"C:/Russell/Projects/Geometry/R_Scripts/Figures/Match_GOSAT_OCO/CSV/OCO3/time_veg_matched_sounding_means_oco3.csv", row.names = FALSE)
write.csv(vza_time_veg_matched_sounding_means,"C:/Russell/Projects/Geometry/R_Scripts/Figures/Match_GOSAT_OCO/CSV/OCO3/vza_time_veg_matched_sounding_means_oco3.csv", row.names = FALSE)
write.csv(n_vza_time_veg_matched_sounding_means,"C:/Russell/Projects/Geometry/R_Scripts/Figures/Match_GOSAT_OCO/CSV/OCO3/n_vza_time_veg_matched_sounding_means_oco3.csv", row.names = FALSE)
write.csv(temp_n_vza_time_veg_matched_sounding_means,"C:/Russell/Projects/Geometry/R_Scripts/Figures/Match_GOSAT_OCO/CSV/OCO3/temp_n_vza_time_veg_matched_sounding_means_oco3.csv", row.names = FALSE)
