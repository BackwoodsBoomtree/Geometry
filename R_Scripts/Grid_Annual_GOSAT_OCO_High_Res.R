...
files_oco2_2020  <- list.files(path = "C:/Russell/Projects/Geometry/Data/oco2/2020", pattern = "*.nc4", full.names = TRUE, recursive = TRUE)

# files_oco3_2019  <- list.files(path = "C:/Russell/Projects/Geometry/Data/oco3/2019", pattern = "*.nc4", full.names = TRUE, recursive = TRUE)
files_oco3_2020  <- list.files(path = "C:/Russell/Projects/Geometry/Data/oco3/2020", pattern = "*.nc4", full.names = TRUE, recursive = TRUE)
...
gosat_annual_2019$SIF_Daily_757nm_Mean <- (gosat_annual_2019$SIF_Daily_757nm_P + gosat_annual_2019$SIF_Daily_757nm_S) / 2
...
raster_gosat_annual_2019 <- rasterize(point_df_gosat_annual_2019, r, field = "SIF_Daily_757nm_Mean", fun = mean)

plot(raster_gosat_annual_2019, zlim = c(0,0.8))

writeRaster(raster_gosat_annual_2019, "C:/Russell/Projects/Geometry/R_Scripts/Figures/Grid_GOSAT_OCO/GOSAT_Annual_Mean_757Daily_2019_0.50.tif", NAflag = -9999, overwrite = TRUE)
...
for (i in 1:length(files_oco2_2020)) {
    date <- substr(basename(files_oco2_2020[i]), 12, 17)
    print(paste0("Working on file number ", i, " and date ", date, "."))

    df <- build_data(files_oco2_2020[i], "OCO")
...
writeRaster(oco2_stack_mean, "C:/Russell/Projects/Geometry/R_Scripts/Figures/Grid_GOSAT_OCO/OCO2_Annual_Mean_757Daily_2020_0.50.tif", NAflag = -9999, overwrite = TRUE)
...
for (i in 1:length(files_oco3_2020)) {
  date <- substr(basename(files_oco3_2020[i]), 12, 17)
  print(paste0("Working on file number ", i, " and date ", date, "."))
  
  df <- build_data(files_oco3_2020[i], "OCO")
  df <- subset_flags(df, NA, 0, 10, "OCO")
...
writeRaster(oco3_stack_mean, "C:/Russell/Projects/Geometry/R_Scripts/Figures/Grid_GOSAT_OCO/OCO3_Annual_Mean_757Daily_2020_0.50.tif", NAflag = -9999, overwrite = TRUE)
