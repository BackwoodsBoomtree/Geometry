...
pair_files <- function(files_oco2, files_oco3) {
  dates_oco2 <- vector()
  dates_oco3  <- vector()
  ...
  files_oco3  <- data.frame(dates_oco3, files_oco3)
    # Pair the filenames
  paired_files <- data.frame(oco2 = vector(), oco3 = vector())
  for (i in 1:nrow(files_oco2)){
    for (j in 1:nrow(files_oco3)){
      if (files_oco2$dates_oco2[i] == files_oco3$dates_oco3[j]){
        paired_files[i, 1] <- files_oco2[i, 2]
...
remove_non_veg <- function (df){
  df <- subset(df, IGBP_index != 13) # Urban
  df <- subset(df, IGBP_index != 15) # Snow
  df <- subset(df, IGBP_index != 16) # Barren
  df <- subset(df, IGBP_index != 17) # Water  
  return(df)
}
...
match_soundings <- function(paired_file_list) {
  ...
  for (i in 1:1){
    date <- substr(basename(paired_file_list$oco2[i]), 12, 17)
    ...
    df_oco2 <- subset_flags(df_oco2, NA, 0, 10, "OCO")
    df_oco3 <- subset_flags(df_oco3, NA, 0, 10, "OCO")
    print("Subset complete.")
    
      # Remove Non veg
    df_oco2 <- remove_non_veg(df_oco2)
    df_oco3 <- remove_non_veg(df_oco3)
    print("Removed Non veg!")

    if (nrow(df_oco2) != 0 && nrow(df_oco3) != 0){
        # Transform dfs into SpatialPolygonsDataframe
      poly_df_oco2 <- build_polyDF(df_oco2)
      print("OCO-2 polydf created.")
      Sys.time()
      poly_df_oco3 <- build_polyDF(df_oco3)
      print("OCO-3 polydf created.")
      Sys.time()
      
        # Do intersections between OCO2 and OCO3
      matched_oco2_sounding_list <- rbind(matched_oco2_sounding_list, as.data.frame(intersect(poly_df_oco2, poly_df_oco3)))
      matched_oco3_sounding_list <- rbind(matched_oco3_sounding_list, as.data.frame(intersect(poly_df_oco3, poly_df_oco2)))

      print(paste0("In total there are now ", nrow(matched_oco2_sounding_list), " overlapping OCO-2 soundings, and ", nrow(matched_oco3_sounding_list), " overlapping OCO-3 soundings."))
    } else {
      print(paste0("Day ", date, " excluded due to empty dataframe. OCO-2 nrow = ", nrow(df_oco2), "; OCO-3 nrow =  ", nrow(df_oco3), "."))
    }
  }
  return(list(matched_oco2_sounding_list, matched_oco3_sounding_list))
}

collapse_soundings <- function(matched_oco2_sounding_list, matched_oco3_sounding_list) {
...
#region ################# RESULTS ##################
paired_file_list <- pair_files(files_oco2, files_oco3)

matched_sounding_list      <- match_soundings(paired_file_list)
matched_oco2_sounding_list <- as.data.frame(matched_sounding_list[1])
matched_oco3_sounding_list <- as.data.frame(matched_sounding_list[2])


unfiltered_matched_sounding_means <- collapse_soundings(matched_oco2_sounding_list, matched_oco3_sounding_list)

veg_matched_sounding_list       <- remove_non_veg(matched_oco2_sounding_list, matched_oco3_sounding_list)
veg_matched_oco2_sounding_list <- as.data.frame(veg_matched_sounding_list[1])
veg_matched_oco3_sounding_list  <- as.data.frame(veg_matched_sounding_list[2])

veg_matched_sounding_means <- collapse_soundings(veg_matched_oco2_sounding_list, veg_matched_oco3_sounding_list)
